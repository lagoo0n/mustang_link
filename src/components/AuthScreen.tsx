...
export default function AuthScreen() {
  ...
  function resolveEmail(raw: string) {
    return raw.includes('@') ? raw : `${raw}@calpoly.edu`;
  }
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const fullEmail = resolveEmail(email.trim());

    if (!fullEmail.endsWith('@calpoly.edu')) {
    ...
    if (mode === 'signup') {
      const { data, error: signUpError } = await supabase.auth.signUp({ email: fullEmail, password });
  ...
  return (
    <div className="min-h-screen bg-[#f9faf8] flex flex-col justify-center px-8 max-w-[430px] mx-auto">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        ...
        <input
          type="text"
          placeholder="Cal Poly username or email"
          value={email}
